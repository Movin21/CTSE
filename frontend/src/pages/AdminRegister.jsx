...
export default function AdminRegister() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0b0f] flex items-center justify-center px-4"
      style={{ backgroundImage: 'radial-gradient(ellipse at 50% 0%, rgba(244,63,94,0.1) 0%, transparent 60%)' }}>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1">Register with elevated privileges</p>
        </div>

        <div className="card border-[rgba(244,63,94,0.2)]" style={{ borderTopColor: '#f43f5e', borderTopWidth: 2 }}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input className="input" type="text" placeholder="Admin username" value={form.username}
                onChange={(e) => setForm({ ...form, username: e.target.value })} required minLength={3} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Email</label>
              <input className="input" type="email" placeholder="admin@ctse.lk" value={form.email}
              ...
              <div className="relative">
                ...
                {form.confirm && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    {form.password === form.confirm ? <CheckCircle2 size={15} className="text-emerald-400" /> : <AlertCircle size={15} className="text-rose-400" />}
                  </div>
                )}
              </div>
            </div>
            <button type="submit" disabled={loading}
              ...
              {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <UserPlus size={16} />}
              {loading ? 'Creating...' : 'Create Admin Account'}
            </button>
          </form>
          <div className="mt-5 pt-4 border-t border-[rgba(244,63,94,0.1)] text-center text-sm text-slate-500">
            Already have an admin account?{' '}
            <Link to="/admin/login" className="text-rose-400 hover:text-rose-300 font-medium">Sign in</Link>
          </div>
        </div>
      </div>
    </div>
