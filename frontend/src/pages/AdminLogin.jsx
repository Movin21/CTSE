    ...
    <div className="min-h-screen bg-[#0a0b0f] flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1">Sign in with administrator credentials</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input className="input pr-10" type={showPassword ? 'text' : 'password'} placeholder="Admin password"
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <LogIn size={16} />}
              {loading ? 'Signing in...' : 'Admin Sign In'}
            </button>
          </form>
          <div className="mt-5 pt-4 border-t border-[rgba(244,63,94,0.1)] text-center text-sm text-slate-500">
            Need an admin account?{' '}
            <Link to="/admin/register" className="text-rose-400 hover:text-rose-300 font-medium">Register</Link>
            <span className="mx-2">·</span>
            <Link to="/login" className="text-indigo-400 hover:text-indigo-300 font-medium">User Login</Link>
          </div>
        </div>
      </div>
    </div>
